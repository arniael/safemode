...
**SafeMode** est un terminal de communication minimaliste conçu pour les personnes neurodivergentes (Autisme, AuDHD etc...). Il sert de "mode de secours" (Safe Boot) lorsque le système sensoriel est en surcharge (Shutdown/Meltdown) et que la communication verbale devient impossible.

---

## 🛠 Le Concept (Analogie Système)

Imaginez que votre cerveau est un serveur sous Linux. Parfois, il y a trop de processus en arrière-plan, trop d'entrées sensorielles (INPUT), ce qui provoque un **Kernel Panic**. 

SafeMode permet de :
* **Couper le flux (L'affichage) :** Une interface sombre (ou claire) et épurée pour limiter la fatigue visuelle.
* **Passer en mode console :** Communiquer par écrit via un chat minimaliste qui permet d'échanger comme sur une conversation SMS (bulle de couleur).
* **Info rapide et efficace :** Un message "popup" pour rapidement expliquer ce qu'il se passe à votre interlocuteur et pourquoi vous souhaitez échanger par écrit.

Cette application est librement inspirée de "Emergency Chat" de Leonard Elezi, aujourd'hui indisponible sur Android.
...
> 
> Faites attention à vous, l'application vous demande de prêter votre téléphone à un possible inconnu pour qu'il puisse vous écrire.
...
Par soucis de transparence, je souhaite préciser que ce projet est conçu par moi même avec l'usage de l'Intelligence Artificielle de Google Gemini 3 Pro qui agit comme partenaire de développement pour la partie "HTML" du projet Cordova.
